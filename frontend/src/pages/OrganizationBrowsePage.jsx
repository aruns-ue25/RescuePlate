import React from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { 
  HeartHandshake, 
  Sparkles, 
  PackageSearch, 
  ArrowRight
} from 'lucide-react';

export default function OrganizationBrowsePage() {
  const { currentUser } = useAuth();

  return (
    <div className="browse-food-page animate-fade-in-up">
      {/* Header Banner */}
      <div className="dashboard-header-banner org-theme-banner">
        <div className="container">
          <div className="badge badge-amber">
            <HeartHandshake size={14} />
            <span>Charity Surplus Food Marketplace</span>
          </div>
          <h1 className="dashboard-title">Browse Available Surplus Food</h1>
          <p className="dashboard-subtitle">
            Connect directly with verified local restaurants, hotels, and bakeries to redistribute fresh surplus food.
          </p>
        </div>
      </div>

      <div className="container dashboard-body" style={{ maxWidth: '820px', margin: '40px auto 80px' }}>
        {/* Coming Soon / Next Sprint Message Card */}
        <div className="profile-card text-center" style={{ padding: '48px 32px' }}>
          <div 
            style={{ 
              width: '72px', 
              height: '72px', 
              borderRadius: '50%', 
              background: 'var(--amber-light)', 
              color: 'var(--amber)', 
              display: 'flex', 
              alignItems: 'center', 
              justifyContent: 'center', 
              margin: '0 auto 20px' 
            }}
          >
            <PackageSearch size={36} />
          </div>

          <div className="badge badge-amber" style={{ marginBottom: '14px' }}>
            <Sparkles size={13} />
            <span>Upcoming Feature • Sprint 2</span>
          </div>

          <h2 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '12px', color: 'var(--text-main)' }}>
            Surplus Food Listings & Claiming Engine
          </h2>

          <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', lineHeight: '1.6', maxWidth: '600px', margin: '0 auto 36px' }}>
            The surplus food discovery, live donor catalog, and partial fulfilment claiming workflows are currently under development for our upcoming sprint.
          </p>

          <div style={{ display: 'flex', justifyContent: 'center', gap: '14px', flexWrap: 'wrap' }}>
            <Link to="/" className="btn btn-outline btn-md">
              <span>Return to Home</span>
            </Link>
            {currentUser ? (
              <Link to="/profile" className="btn btn-amber btn-md">
                <span>Manage Your Profile</span>
                <ArrowRight size={16} />
              </Link>
            ) : (
              <Link to="/register" className="btn btn-amber btn-md">
                <span>Register Organization</span>
                <ArrowRight size={16} />
              </Link>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}
